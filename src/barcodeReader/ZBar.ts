...
import { Symbol as ZBarSymbol, scanImageData, Point as ZBarPoint } from 'zbar.wasm';
...
export default class ZBar {
  ...
  wrapResult(result:ZBarSymbol):BarcodeResult {
  ...
  getRectFromPoints(resultPoints:ZBarPoint[]):Rect {
    return getRectFromPoints(this.convertZBarPointsToPoints(resultPoints))
  }

  convertZBarPointsToPoints(resultPoints:ZBarPoint[]):Point[] {
    const points = [];
